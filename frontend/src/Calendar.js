...
export default function Calendar() {
    ...
    // Mock Streak Data (would come from context/DB)
    const STREAK_DAYS = [
        new Date().toISOString().split('T')[0], // Today
        new Date(Date.now() - 86400000).toISOString().split('T')[0], // Yesterday
        // Add more mock days if needed for visual verification
    ];
        ...
        <div className="flex flex-col lg:flex-row h-[calc(100vh-2rem)] gap-6 p-4 overflow-hidden">
            ...
            <aside className="w-full lg:w-1/3 flex flex-col gap-6 h-full overflow-y-auto no-scrollbar">
                ...
                <div className="flex items-center justify-between mb-2">
                    ...
                    <button className="w-10 h-10 bg-white rounded-xl shadow-sm border border-gray-100 flex items-center justify-center text-gray-600 hover:text-[#1dc962] transition-colors">
                        <FaPlus />
                    </button>
                </div>
